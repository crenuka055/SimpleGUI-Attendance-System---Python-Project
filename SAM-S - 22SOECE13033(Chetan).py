...
#Group Project: Chetan, Vishwajeet, Ajit
...
def DisplayForm():
    ...
    display_screen.title("Student Attendence Management System")
    #declaring variables
    global tree
    global SEARCH
    ...
    LeftViewForm = Frame(display_screen, width=500,bg="grey")
...
def DisplayForm():
    ...
    display_screen.geometry("900x400")
    #setting title for window
    display_screen.title("SAM System | Python Project - Chetan")
    global tree
    global SEARCH
    global name,contact,email,rollno,branch
    SEARCH = StringVar()
    ...
    MidViewForm.pack(side=RIGHT)
    #label for heading
    lbl_text = Label(TopViewForm, text="Student Attendance Records", font=('verdana', 18), width=600,bg="#1C2833",fg="white")
    lbl_text.pack(fill=X)
    #creating registration form in first left frame
    Label(LFrom, text="Name  ", font=("Arial", 12)).pack(side=TOP)
    Entry(LFrom,font=("Arial",10,"bold"),textvariable=name).pack(side=TOP, padx=10, fill=X)
    ...
    Label(LFrom, text="Branch ", font=("Arial", 12)).pack(side=TOP)
    Entry(LFrom, font=("Arial", 10, "bold"),textvariable=branch).pack(side=TOP, padx=10, fill=X)
    Button(LFrom,text="Attended",font=("Arial", 10, "bold"),command=register).pack(side=TOP, padx=10,pady=5, fill=X)
